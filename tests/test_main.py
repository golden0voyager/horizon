...
from src.main import main, print_config_template
...
def _patch_main(monkeypatch: pytest.MonkeyPatch, fake_orchestrator: MagicMock) -> None:
    ...
    monkeypatch.setattr(
        "src.main.HorizonOrchestrator", lambda *_a, **_kw: fake_orchestrator
    )
...
def test_main_runs_orchestrator_with_no_args(
    ...
    monkeypatch.setattr(
        "src.main.argparse.ArgumentParser.parse_args",
        lambda *_a, **_kw: argparse.Namespace(hours=None),
    )
...
def test_main_passes_force_hours_to_orchestrator(
    minimal_config: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    fake_run = AsyncMock(return_value=None)
    fake_orchestrator = MagicMock()
    ...
    monkeypatch.setattr(
        "src.main.argparse.ArgumentParser.parse_args",
        lambda *_a, **_kw: argparse.Namespace(hours=12),
    )
...
def test_main_exits_when_config_missing_without_template(
...
def test_main_exits_when_config_missing_with_template(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_main_generic_orchestrator_exception_exits_one(
    minimal_config: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    fake_orchestrator = MagicMock()
